use cxdb;
SELECT D.[Name] AS ProjectName, c.ScanCompletedOn, c.projectid, A.ScanId, B.Name as ResultState, a.severity, ss.name as Query, qg.Name as Language, count(a.id) as Count
       ...
       A.scanid in 
		(
			...
			from 
				taskscans as ts 
		 where 
				ts.projectid not in (1,2,3,6,7,8,9)
...
group by 
	D.[Name], c.ScanCompletedOn, c.projectid, A.ScanId, B.Name, a.severity, ss.name, qg.name
order by 
